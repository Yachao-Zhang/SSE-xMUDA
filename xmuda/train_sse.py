...
from xmuda.models.build_sse import build_model_2d, build_model_3d
from xmuda.data.build import build_dataloader
from xmuda.data.utils.validate_sse import validate
from xmuda.models.losses import entropy_loss
from xmuda.data.utils.data_mixaugment_dd import data_mixaugment
from xmuda.models.prototype_generator_ssd import propotype_generator
import random
def parse_args():
    ...
    parser.add_argument(
        '--cfg',
        dest='config_file',
        default='../configs/nuscenes/day_night/xmuda.yaml',
        metavar='FILE',
...
def train(cfg, output_dir='', run_name=''):
    ...
    if cfg.dis:
        num_class = cfg.MODEL_2D.NUM_CLASSES
        class_prototype_bank_2d = torch.zeros(num_class, 64).cuda()
        class_prototype_bank_3d = torch.zeros(num_class, 16).cuda()
        pseclass_prototype_bank_2d = torch.zeros(num_class, 64).cuda()
        pseclass_prototype_bank_3d = torch.zeros(num_class, 16).cuda()
    ...
    for iteration in range(start_iteration, max_iteration):
        ...
        data_batch_src['istrg'] = False
        data_batch_trg['istrg'] = True
        data_batch_trg['cut_W'] = None
        data_batch_src['inter'] = iteration
        data_batch_trg['inter'] = iteration
        data_batch_src['dis'] = cfg.dis
        data_batch_trg['dis'] = cfg.dis
        data_batch_src['mix'] = cfg.mix
        data_batch_trg['mix'] = cfg.mix
        data_batch_src['ssp'] = cfg.ssp
        data_batch_trg['ssp'] = cfg.ssp
        ...
        # ---------------------------------------------------------------------------- #
        if cfg.dis:
            data_batch_src['2dproto'] = pseclass_prototype_bank_2d
            data_batch_src['3dproto'] = pseclass_prototype_bank_3d

        if cfg.mix:#
            init_cut_redio =  random.choice([6 / 10, 7 / 10, 8 / 10, 9/10])
            paced = 0
            data_batch_src = data_mixaugment(data_batch_src, data_batch_trg, init_cut_redio, paced) #
            ...
            data_batch_src['cut_W'] = (data_batch_src['img'].shape[-1])//2
            # print((data_batch_src['img'].shape[-1])//2)
            preds_2d = model_2d(data_batch_src)
            preds_3d = model_3d(data_batch_src)
        ...
        if cfg.dis:
            class_prototype_bank_2d, class_prototype_bank_3d = propotype_generator(preds_2d['feats'],
                                                                               preds_3d['feats'],
                                                                               class_prototype_bank_2d,
                                                                               class_prototype_bank_3d,
                                                                               data_batch_src['seg_label'],
                                                                               num_class)
        if cfg.ssp:
            self_loss_src_2d =  0.01 * preds_2d['posi_loss']
            self_loss_src_3d = 0.01* preds_3d['self_loss_3d']
            train_metric_logger.update(self_loss_src_2d=self_loss_src_2d, self_loss_src_3d=self_loss_src_3d)
            loss_2d += self_loss_src_2d
            loss_3d += self_loss_src_3d
        ...
        if cfg.dis and iteration>20000:
            sty_loss_src_2d = 0.05 * F.cross_entropy(preds_2d['con_all_fea'], preds_2d['all_label'].detach())
            sty_loss_src_3d = 0.05 * F.cross_entropy(preds_3d['con_all_fea'], preds_3d['all_label'].detach())
            loss_2d += sty_loss_src_2d
            loss_3d += sty_loss_src_3d
            print('SRC：', sty_loss_src_2d, sty_loss_src_3d)
        ...
        # ---------------------------------------------------------------------------- #
        if cfg.dis:
            data_batch_trg['2dproto'] = class_prototype_bank_2d
            data_batch_trg['3dproto'] = class_prototype_bank_3d

        preds_2d = model_2d(data_batch_trg)
        preds_3d = model_3d(data_batch_trg)
        if cfg.dis and iteration>1:
            pseclass_prototype_bank_2d, pseclass_prototype_bank_3d = propotype_generator(preds_2d['feats'],
                                                                                   preds_3d['feats'],
                                                                                   pseclass_prototype_bank_2d,
                                                                                   pseclass_prototype_bank_3d,
                                                                                   preds_2d['pseudo_label'],
                                                                                   num_class)
        ...
        if cfg.ssp:
            self_loss_trg_2d = 0.01 * preds_2d['posi_loss']#
            self_loss_trg_3d = 0.01 * preds_3d['self_loss_3d']#
            train_metric_logger.update(self_loss_trg_2d=self_loss_trg_2d, self_loss_trg_3d=self_loss_trg_3d)
            loss_2d.append(self_loss_trg_2d)
            loss_3d.append(self_loss_trg_3d)
        ...
        if cfg.dis and iteration>20000:
            sty_loss_trg_2d = 0.01 * F.cross_entropy(preds_2d['con_all_fea'], preds_2d['all_label'].detach())
            sty_loss_trg_3d = 0.01 * F.cross_entropy(preds_3d['con_all_fea'], preds_3d['all_label'].detach())

            loss_2d.append(sty_loss_trg_2d)
            loss_3d.append(sty_loss_trg_3d)
