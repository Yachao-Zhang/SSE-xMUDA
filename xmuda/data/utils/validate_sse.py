...
def validate(cfg,
    ...
    with torch.no_grad():
        for iteration, data_batch in enumerate(dataloader):
            ...
            data_batch['ssp'] = False
            data_batch['istrg'] = True
            data_batch['cut_W'] = None
            data_batch['inter'] = 0
            data_batch['dis'] = False
            data_batch['mix'] = False
            data_batch['rol'] = None
            data_batch['inter'] = 0
            # predict
            ...
            for batch_ind in range(len(seg_label)):
                ...
                # pred_label_mask = pred_label_value_mask[left_idx:right_idx] if model_3d else None
                # evaluate
                ...
                # igt = './gt/' + str(it)
                # ipd = './pd/' + str(it)
                # if not os.path.exists('./gt/'):  # 判断是否存在文件夹如果不存在则创建为文件夹
                #     os.makedirs('./gt/')
                # if not os.path.exists('./pd/'):  # 判断是否存在文件夹如果不存在则创建为文件夹
                #     os.makedirs('./pd/')
                ...
                # draw_points_image_labels(data_batch['img'][batch_ind].permute(1,2,0).cpu(), img_indices[batch_ind], curr_seg_label,igt,
                #                          color_palette_type='NuScenes', point_size=3)
                # draw_points_image_labels((data_batch['img'][batch_ind].permute(1, 2, 0)+.50).cpu(), img_indices[batch_ind],
                #                          pred_label_ensemble, ipd, color_palette_type='NuScenes', point_size=3)
